...
class DisplayQueue extends Component {
  ...
  async refreshQueue() {
    let ticketsWithDoctors = (
      await axios.get(`${this.URL}/queues/getticketswithdoctors`)
    ).data;
    this.setState({
      ticketsWithDoctors,
    });
    ...
    return (
      <div className="container">
        <div className="row" style={{ marginTop: "20px" }}>
          {this.state.ticketsWithDoctors.length === 0 &&
            "No patient is currently being attended by doctors."}
          {latestTicketWithDoctor && (
            <div
              className="col-lg-12 card text-center"
              style={{ height: "250px" }}
            >
        ...
        <div className="row" style={{ marginBottom: "20px" }}>
          {latestTicketWithDoctor &&
            this.state.ticketsWithDoctors.map((ticketWithDoctor) => (
              <div
                key={ticketWithDoctor.ticketNumber}
